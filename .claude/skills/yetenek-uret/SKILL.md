...
- **BkmArgus-adapte ZORUNLU** (jenerik şablonu körü körüne wire ETME): güvenlik (şifre/`ex.Message`/boş catch) = BLOK; stil/konvansiyon (inline-style, magic-string) = UYAR. `DateTime.Now` BkmArgus'ta yasak (timezone) → bunu BLOK değil belirgin uyar.
...
- BkmArgus'ta eval-viewer altyapısı yok; kıyas + assertion grade'i konuşmada raporla, sonucu journal'a not et.
