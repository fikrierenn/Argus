...
description: BkmArgus oturum sonu özeti yazar. Bugün yapılanları, build durumunu, yarım kalan işleri, yarına başlangıç noktasını docs/journal/YYYY-MM-DD.md dosyasına yazar. Yazım sonunda journal'i otomatik commit eder (sadece journal dosyası — başka dosyaya dokunmaz). Kullanıcı "handoff", "oturum sonu", "iyi geceler", "kaydet ve kapat", "günaydın özet" gibi ifadeler kullandığında veya /handoff çalıştırıldığında devreye gir.
allowed-tools: Read, Edit, Write, Bash, Grep, Glob
user-invocable: true
model: inherit
---

# BkmArgus Oturum Devir Skill'i
...
### Adım 2 — Build Durumu
```bash
dotnet build src/BkmArgus.Web/BkmArgus.Web.csproj 2>&1 | tail -3
```
...
5. **KALAN İŞLER DETAYLI companion ZORUNLU.** Journal'daki tek-satır "Yarım Kalan" YETMEZ. Ayrı bir
   `docs/journal/YYYY-MM-DD-kalan-isler.md` yazılır ve HER açık iş için şu şema doldurulur:
   **NE · NEDEN · NEREDE (dosya:satır) · NASIL KAPANIR · BAĞIMLILIK · ÖNCELİK.** Entegrasyon işi
   (ör. ENS gömme) karşı-repoyu da içeriyorsa iki journal birbirine referans verir.
   > **Bu kural bir unutmadan doğdu (2026-07-29, ENS entegrasyon oturumu).** "Kalanları detaylı yaz,
   > unutuyorsun" denildi. Detaysız borç bir sonraki oturumda kaybolur; companion = envanter.
