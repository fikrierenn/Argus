...
## BkmArgus Mevcut Durum (2026-05-28)

- `src/BkmArgus.Cli/` — manuel test (CLI komutları)
- `src/BkmArgus.Web/` — **otomatik test yok** (henüz)
- SQL SP test: manuel via `sqlcli query`
- Smoke test: tarayıcıdan manuel

**Acil eklenmesi gerekenler:**
1. `src/BkmArgus.Tests/` xUnit projesi
...
```
fix(etl): DailyProductRisk PK bug

Test: dotnet test → 0/0 (manual smoke OK)
SP test: sqlcli query "SELECT ... FROM v_AccountBalance" -> doğru bakiye
```
