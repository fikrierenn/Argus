...
Bu dosya, BkmArgus projesinin Git versiyon kontrol sistemi kullanımını, commit disiplinini ve uncommitted dosya limitlerini tanımlar. Temiz bir kod geçmişi ve güvenli geri alma (rollback) noktaları oluşturmak için bu kurallar uygulanır.
...
*   **Bir TODO maddesini kapatan commit atılırken AYNI ANDA o madde `TODO.md`'de `[x] ✅ <tarih> (commit <hash>)` yapılır.** Commit atıp TODO'yu açık bırakmak yasak — "done ama `[ ]`" birikiminin kök sebebi budur. İstisna yok.
