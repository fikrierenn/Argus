...
Bu kural BkmArgus üzerindeki her oturuma uygulanır. `paths:` filtre yoktur — compact ve clear sonrası da etkilidir.
...
| Tier | Tanım | Plan? | BkmArgus örnek |
|---|---|---|---|
...
| **3 — Substantial** | 3+ dosya yeni pattern, schema/security/UX/harici bağımlılık, kullanıcı-görünür değişiklik | **TAM PLAN ZORUNLU** (`plans/NN-<slug>.md`) | Yeni modül, `sql/NN_*.sql` schema migration, ETL değişikliği, RBAC/auth değişikliği, yeni AI skill+job zinciri, çoklu ekran refactor (Ref/Index split) |

## Tier 3 sinyalleri

Şu sinyallerden BİRİ varsa Tier 3 sayılır:

1. **3+ farklı klasöre dokunma** (örn. `sql/` + `src/BkmArgus.Web/` + `.claude/`)
2. **Yeni dosya tipi** (yeni `sql/NN_*.sql`, yeni SP grubu, yeni `Features/<Modül>/` klasörü, yeni `Jobs/*Job.cs`)
3. **Geri alınması zor** (DB migration, kolon silme, FK constraint ekleme)
4. **Kullanıcı-görünür** UI/UX/sidebar/route değişiklik
5. **Harici bağımlılık** (yeni NuGet paketi, yeni LLM sağlayıcı, yeni env var, yeni Ollama modeli)
6. **Mimari karar** (yeni pattern, yeni standart, ADR seviyesi karar)

7. **`src.*` view'a veya `rpt.*` snapshot şemasına dokunma** — ERP/ETL zinciri kırılabilir
8. **AI maliyet yüzeyi** (yeni LLM çağrısı, yeni skill, yeni job zamanlaması)
...
- Her commit message'da plan referansı: `feat(dof): SLA eskalasyon (plan: 03)`
...
- `docs/MASTER_PLAN.md` — modül öncelik sıralaması
- `.claude/rules/footprint-ladder.md` — Tier 3'e çıkmadan önce dar basamak elendi mi
