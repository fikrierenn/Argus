#!/usr/bin/env bash
# Post-edit antipattern WARN — BkmArgus PostToolUse hook (Edit|Write).
#
...
if [[ "$f" == *.cs ]]; then
  ...
  grep -qnE 'DataTable\.Compute' "$f" 2>/dev/null && warns+=("DataTable.Compute yasak (formula injection)")
  grep -qnE '(sk-ant-api[0-9]{2}-|AIza[0-9A-Za-z_-]{20,})' "$f" 2>/dev/null && warns+=("API key hardcoded -> appsettings.Local.json")
...
if [[ "$f" == *.cshtml ]]; then
  grep -qnE 'style="[^"]*(color|font-|background-color|border-color|box-shadow)' "$f" 2>/dev/null && warns+=("Inline style (renk/font) -> Tailwind class")
  grep -qn '@Html.Raw' "$f" 2>/dev/null && warns+=("@Html.Raw XSS riski")
  grep -q '^@page' "$f" 2>/dev/null && ! grep -q 'Authorize' "$f" 2>/dev/null && warns+=("@page var, [Authorize] yok")
fi

if [[ "$f" == *.sql ]]; then
  grep -qnE 'GETDATE\s*\(' "$f" 2>/dev/null && warns+=("GETDATE() -> SYSDATETIME()")
  grep -qniE '(float|real)' "$f" 2>/dev/null && warns+=("float/real -> decimal")
  grep -qniE 'SELECT[[:space:]]+\*' "$f" 2>/dev/null && warns+=("SELECT * yasak")
fi
