...
description: BkmArgus kodunda silent failure, yetersiz error handling, uygunsuz fallback davranışı tespit eder. Catch block / fallback / error suppression içeren değişikliklerden sonra proaktif çağır.
...
## BkmArgus Kuralları (her zaman uygula)
...
- Technical detail uygun şekilde gizleniyor mu? (BkmArgus: `ex.Message` user'a YASAK)
...
### 5. BkmArgus Standartlarına Karşı Doğrula
...
- `AiWorker` job'larında (`Jobs/*Job.cs`) exception → job sessizce atlanır; `ai.AgentExecutions`'a yazılmazsa iz kalmaz
- LLM sağlayıcı zincirinde soft-fail: kapalı/başarısız sağlayıcı atlanırken **neden atlandığı** loglanmalı
