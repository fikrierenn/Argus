...
## BkmArgus Bucket Pattern'leri

| Pattern | Bucket |
|---|---|
| `sql/NN_*.sql` | Schema migration — ilgili modülün UI/SP'leri ayrı bucket |
| `sql/db_objects*.sql` | Stored Procedure / View — ayrı bucket |
| `sql/seed_*.sql` | Seed verisi — şema ile beraber olabilir |
| `src/BkmArgus.Web/Lib/*.cs` | Çekirdek kütüphane değişiklikleri |
| `src/BkmArgus.Web/wwwroot/css/parts/*` | UI standart parça |
| `src/BkmArgus.Web/wwwroot/css/input.css` + `site.css` | CSS rebuild — parts ile beraber |
| `src/BkmArgus.Web/Program.cs` | Pipeline/DI değişiklikleri |
...
| `src/BkmArgus.Cli/Program.cs` | CLI değişiklikleri |
...
- `TODO.md` — aktif feature durumları
