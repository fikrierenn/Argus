...
IF OBJECT_ID(N'ai.AiLmSonuc', N'U') IS NULL
BEGIN
    CREATE TABLE ai.AiLmSonuc
    (
        IstekId         bigint NOT NULL PRIMARY KEY,
        RootCauseClass  varchar(50) NOT NULL,
        EvidencePlan    varchar(20) NOT NULL,
        LlmGerekliMi    bit NOT NULL,
        OncelikPuan     int NOT NULL,
        KisaOzet        nvarchar(1000) NULL,
        OzellikJson     nvarchar(max) NULL,
        RuleSetVersiyon varchar(20) NULL,
        OlusturmaTarihi datetime2(0) NOT NULL CONSTRAINT DF_AiLmSonuc_Tarih DEFAULT (SYSDATETIME()),
        CONSTRAINT FK_AiLmSonuc_Istek FOREIGN KEY (IstekId) REFERENCES ai.AiAnalizIstegi(IstekId)
    );
END
GO

IF OBJECT_ID(N'ai.AiAnalizSonucu', N'U') IS NULL
BEGIN
    CREATE TABLE ai.AiAnalizSonucu
    (
        SonucId         bigint IDENTITY(1,1) NOT NULL PRIMARY KEY,
        IstekId         bigint NOT NULL,
        ModelAdi        varchar(60) NOT NULL,
        Ozet            nvarchar(1000) NOT NULL,
        KokNeden        nvarchar(max) NULL,
        OneriAksiyon    nvarchar(max) NULL,
        GuvenPuani      decimal(5,2) NULL,
        HamCikti        nvarchar(max) NULL,
        OlusturmaTarihi datetime2(0) NOT NULL CONSTRAINT DF_AiAnalizSonucu_Tarih DEFAULT (SYSDATETIME()),
        CONSTRAINT FK_AiAnalizSonucu_Istek FOREIGN KEY (IstekId) REFERENCES ai.AiAnalizIstegi(IstekId)
    );
END
GO

IF OBJECT_ID(N'ai.AiGeriBildirim', N'U') IS NULL
BEGIN
    CREATE TABLE ai.AiGeriBildirim
    (
        Id              bigint IDENTITY(1,1) NOT NULL PRIMARY KEY,
        SonucId         bigint NOT NULL,
        DogruMu         bit NOT NULL,
        Notlar          nvarchar(max) NULL,
        Veren           nvarchar(160) NOT NULL,
        Tarih           datetime2(0) NOT NULL CONSTRAINT DF_AiGeriBildirim_Tarih DEFAULT (SYSDATETIME()),
        IstekId         bigint NULL,
        DenetciKarari   varchar(20) NULL,
        GercekKokNeden  varchar(50) NULL,
        DenetciNotu     nvarchar(2000) NULL,
        KapanisDT       datetime2(0) NULL,
        CONSTRAINT FK_AiGeriBildirim_Sonuc FOREIGN KEY (SonucId) REFERENCES ai.AiAnalizSonucu(SonucId),
        CONSTRAINT FK_AiGeriBildirim_Istek FOREIGN KEY (IstekId) REFERENCES ai.AiAnalizIstegi(IstekId)
    );
END
