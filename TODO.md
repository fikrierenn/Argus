...
- [ ] **B20. ETL veri kalitesi kapısını REDDET / SAY diye böl** — merkezî `error-handling.md`'nin "Doğrulama Sınırının Ölçütü" maddesi (2026-09-23 terfi): **çelişki → REDDET, eksiklik → SAY**. Tek soru: *"bu iki şey aynı anda doğru olabilir mi?"* Bizim `etl-discipline.md §6` hepsini tek kovaya koyuyor (*"Bu kayıtlar ETL'i durdurmaz ama raporlanır"*). Listemizde **negatif stok** fiziksel olarak imkânsızdır, yani çelişkidir ve ETL'i durdurmalı; **eşleşmeyen mekan/ürün** gerçekten eksikliktir, SAY doğru. `ehAltDepo <> 0` zaten P0 alarmı — hangi kovada olduğu karara bağlanmalı. **Davranış değişikliği**: `bkmargus-etl` danışmanı + plan gerektirir, tek başına dosya düzenlemesi değil.
- [ ] **B21. `Ref/Index.cshtml.cs:793` N+1 yazma döngüsü** — `foreach (var tipId in tipIds) { await _db.ExecuteAsync(...) }`, kayıt başına bir round-trip. Depodaki **tek** N+1 (tam tarama yapıldı). Okuma değil toplu yazma olduğu için etkisi sınırlı; TVP ya da tek SP çağrısına indirilmeli. Merkezî `performance.md`'nin bu depoda yakaladığı tek somut madde.
  - **`performance.md` KOPYALANMADI, bilinçli:** merkez "kuralı kopyalama, işaret et" diyor ve `CLAUDE.md` başındaki işaretçi onu zaten kapsıyor. Kopyalamak 170'inci sapma adayı üretirdi — kuralın kendisini çiğneyerek kural almak olurdu.
- [ ] **B22. `claude-context-template` → `Norma` yol düzeltmesi** — merkez deposu yeniden adlandırıldı; eski ad **junction** olarak duruyor, yani bugün hiçbir şey kırılmıyor. Junction kalıcı değil. Bizde düzeltilecek dosyalar: `.claude/skills/cati-degerlendirme/SKILL.md` · `CLAUDE.md` işaretçisi · `.claude/MERKEZ-BILDIRIMI.md`. Junction silinmeden önce yapılmalı.
